...
def estimated_power(df: pd.DataFrame, start_time: int, end_time: int, rider_weight: float, bike_weight: float,
    ...
    df['rolling_force'] = (np.cos(np.arctan(df.slope)) * 9.8067 * (
                bike_weight + rider_weight) * rolling_resistance) * df.speed
    df['speed_change'] = (bike_weight + rider_weight) * (df.speed.diff() / df.seconds.diff()) * df.speed
    df['vam_power'] = (df[['air_drag', 'climbing_force', 'rolling_force', 'speed_change']].sum(axis='columns'))
    return df
