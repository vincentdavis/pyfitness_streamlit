...
if fit_file is not None:
    ...
    if not (
        all([field in df.columns for field in required_fields2])
        or not all([field in df.columns for field in required_fields2])
    ):
        st.write("### Missing at least 1 required fields")
        st.write("power, (enhanced)_altitude, distance, (enhanced)_speed")
        st.write(f"Available fields: {df.columns}")
        st.write("Found the following fields:")
        ...
        try:
            ...
            with t1:
                start_time = st.number_input(
                    "Start time sec:",
                    min_value=0,
                    max_value=int(df["seconds"].max()),
                    value=int(min5["start_time"]),
                    step=1,
                )
            with t2:
            ...
            with c3:
                ...
                drag_coefficient = st.number_input(
                    "Drag coefficient:", min_value=0.0, max_value=1.0, value=0.8, step=0.05
                )
            with c4:
                ...
                rolling_resistance = st.number_input(
                    "Rolling resistance:", min_value=0.000, max_value=0.1, value=0.005, step=0.001, format="%.3f"
                )
            with c5:
                efficiency_loss = st.number_input(
                    "Efficiency: drivetrain, road, ... 0.05 = 5%", min_value=0.0, max_value=1.0, value=0.04, step=0.01
                )
                roll = st.number_input("Smooting:", min_value=0, max_value=30, value=15, step=1)
            avg_est_power = climb_power_estimate(
                df=df_filtered,
            ...
            with st.expander("stat info", expanded=False):
                st.write(
                    "- Average measured power: The average power from your FIT file for the selected segment\n"
            ...
            fig = go.Figure()
            pvam_fig = go.Figure()
            pvam_fig.update_layout(title="Measured vs Estimated Power")
            pvam_fig.add_trace(
            ...
            pvam_fig.add_trace(
                go.Scatter(
                    name="Est. Power (point2point)",
                    x=[start_time, end_time],
                    y=[avg_est_power["est_power"], avg_est_power["est_power"]],
                )
            )
            pvam_fig.add_trace(
                go.Scatter(
                    name="Avg Power", x=[start_time, end_time], y=[fitted["power"].mean(), fitted["power"].mean()]
                )
            )
            pvam_fig.add_trace(
                go.Scatter(
            ...
            comp_fig.add_trace(
                go.Scatter(
                    name="Acceleration", x=fitted["seconds"], y=fitted["acceleration_watts"].rolling(roll).mean()
                )
            )
            pvam_fig.update_layout(title="Components or Estimated VAM Power")
            ...
            st.download_button(
                label="Download FIT file and power estimates as csv for the segment",
            ...
            st.write("Take a look at the data below and the error in red below that.")
            st.write("You can message me on discord [VincentDavis#3484](discordapp.com/users/VincentDavis#3484)")
